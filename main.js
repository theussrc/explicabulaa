...
async function loadData() {
    try {
        ...
        console.error('Falha ao carregar db.json:', error);
    }
}
...
function initCurrentPage() {
    ...
    if (path.includes('index') || path.endsWith('/')) {
        initHome();
    } else if (path.includes('medicamentos.html')) {
        initMedicamentos();
    } else if (path.includes('sintomas.html')) {
        initSintomas();
    } else if (path.includes('bula.html')) {
        initBula();
    }
...
function handleSearch(query) {
    ...
    window.location.href = `medicamentos.html?q=${encodeURIComponent(query)}`;
}
...
function initHome() {
    ...
    if (categoriesContainer && appData.categorias) {
        categoriesContainer.innerHTML = appData.categorias.map(cat => `
            <a href="medicamentos.html?q=${encodeURIComponent(cat.nome.split(' ')[0])}" class="category-card">
                <span>${cat.nome}</span>
    ...
    if (medicinesContainer && appData.medicamentos) {
        medicinesContainer.innerHTML = appData.medicamentos.slice(0, 3).map(med => createMedicineCard(med)).join('');
    }
    
    if (symptomsContainer && appData.sintomas) {
        symptomsContainer.innerHTML = appData.sintomas.slice(0, 5).map(sintoma => `
            <a href="sintomas.html?id=${sintoma.id}" style="background: var(--background); padding: 12px 24px; border-radius: 30px; font-weight: 500; border: 1px solid var(--border);">${sintoma.nome}</a>
        `).join('');
    }
}
...
function createMedicineCard(med) {
    const categoria = appData.categorias.find(c => c.id === med.categoria)?.nome || 'Medicamento';
    return `
        <a href="bula.html?id=${med.id}" class="card">
...
// Lógica das outras páginas (serão preenchidas nos próximos passos)
function initMedicamentos() {
    ...
    function renderList(filterText = '') {
        if (!listContainer || !appData.medicamentos) return;
        
        const text = filterText.toLowerCase();
        const filtered = appData.medicamentos.filter(med => {
            const cat = appData.categorias.find(c => c.id === med.categoria)?.nome || '';
            return med.nome.toLowerCase().includes(text) || cat.toLowerCase().includes(text);
        });
...
function initSintomas() {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const idSintoma = urlParams.get('id');
    
    if (!listContainer || !appData.sintomas) return;
    
    function renderSintomasList() {
        listContainer.innerHTML = appData.sintomas.map(sintoma => `
            <li>
                <a href="#" onclick="selectSintoma('${sintoma.id}', event)" 
                   style="display: block; padding: 12px 16px; border-radius: 8px; background: ${sintoma.id === idSintoma ? 'var(--primary)' : 'white'}; color: ${sintoma.id === idSintoma ? 'white' : 'var(--text-main)'}; box-shadow: var(--shadow-sm); border: 1px solid var(--border);">
                   ${sintoma.nome}
                </a>
            </li>
        `).join('');
    ...
    window.selectSintoma = (id, event) => {
        ...
        renderSintomasList(); // Re-renderiza para atualizar a classe active
    ...
    function showMedicamentosDoSintoma(id) {
        ...
        resultContainer.innerHTML = `
            <h2>Medicamentos para: <span style="color: var(--primary);">${sintoma.nome}</span></h2>
    ...
    if(sidebar) {
        ...
        if(sintomasDesseMed.length > 0) {
            sidebar.innerHTML = sintomasDesseMed.map(s => `
                <li>
                    <a href="sintomas.html?id=${s.id}" style="background: #f1f5f9; padding: 6px 12px; border-radius: 20px; font-size: 13px; font-weight: 500;">
                        ${s.nome}
                    </a>
                </li>
            `).join('');
